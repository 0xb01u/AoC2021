...
def explore(node, caves, small_caves, explored, path, paths):
	for nxt in caves[node]:
		if nxt == 'end':
			...
			explore(nxt, caves, small_caves, explored_nxt, path + f",{nxt}", paths)

paths = []
explore('start', connections, small_caves, set(), 'start', paths)
print(f"Amount of paths that only visit small caves at least once: {len(paths)}")
assert len(paths) == 4104
