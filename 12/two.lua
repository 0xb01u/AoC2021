...
function explore(node, caves, small_caves, explored, path, paths, twice)
	twice = twice or false
	for nxt, _ in pairs(caves[node]) do
		...
		elseif not explored[nxt] then
			...
			explore(nxt, caves, small_caves, explored_nxt, path .. "," .. tostring(nxt), paths, twice)
		elseif not twice then
			explored_nxt = {}
			for cave, _ in pairs(explored) do
				explored_nxt[cave] = true
			end
			explore(nxt, caves, small_caves, explored_nxt, path .. "," .. tostring(nxt), paths, true)
		end
	end
end
...
print("Amount of paths that only visit small caves at least once and maybe one twice: " .. #paths)
assert(#paths == 119760)
