...
for i = 1, #first_x do
	x1, x2 = first_x[i], second_x[i]
	y1, y2 = first_y[i], second_y[i]
	...
	elseif y1 == y2 then
		...
		_y = x1 == min_x and y1 or y2
		inc = _y == min_y and 1 or -1
		for i = 0, max_x - min_x do
			dx = min_x + i
			dy = _y + i * inc
			points[dx][dy] = points[dx][dy]- -1
		end
	end
end
